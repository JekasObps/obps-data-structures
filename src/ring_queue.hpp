...
public:
    explicit RingQueue(size_t queue_size);
    ~RingQueue();
...
template <size_t msg_size>
RingQueue<msg_size>::RingQueue(size_t queue_size)
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::ReadTo(char *out)
{
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::ReadTo(std::ostream& dest)
{
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::ReadTo(UserReadCallback callback)
{
...
template<size_t msg_size>
template<typename M>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::ReadEmplace(M * memory)
{
...
template<size_t msg_size>
template<typename M>
void RingQueue<msg_size>::Construct(M * memory, const char* buffer) noexcept
{
    new (memory) M(*reinterpret_cast<const M*>(buffer));
}

template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::WriteFrom(const char *in, uint16_t size)
{
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::WriteFrom(std::istream& in_stream, uint16_t size)
{
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::WriteFrom(UserWriteCallback callback)
{
...
template<size_t msg_size>
template<typename M, typename ...Args>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::WriteEmplace(Args&& ...args)
{
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::ReadDecorator(const std::function<void(void)> read_impl)
{
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::ReadCriticalSection(const std::function<void(void)> &read_impl)
{
    if (isReadAvailable())
    {
...
template <size_t msg_size>
RingQueue<msg_size>::OperationStatus RingQueue<msg_size>::WriteDecorator(std::function<void(void)> write_impl)
{
    for(;;) {
...
template <size_t msg_size>
bool RingQueue<msg_size>::isAlive() const noexcept
{ 
    return m_Alive;
}

template <size_t msg_size>
void RingQueue<msg_size>::ShutDownReaders() 
{ 
...
template <size_t msg_size>
void RingQueue<msg_size>::ShutDownWriters() noexcept
{ 
    m_ShutDownWriters = true;
}

template <size_t msg_size>
void RingQueue<msg_size>::ShutDown() 
{ 
    ShutDownWriters();
    ShutDownReaders();
...
template <size_t msg_size>
bool RingQueue<msg_size>::isReadAvailable() const noexcept
{
    return GetMessagesCount() > 0; 
}

template <size_t msg_size>
bool RingQueue<msg_size>::isWriteAvailable() const noexcept
{
    return GetMessagesCount() < m_QueueSize;
}

template <size_t msg_size>
size_t RingQueue<msg_size>::GetMessagesCount() const noexcept 
{
    return messages_available_for_reading.load(std::memory_order_relaxed);
}

template <size_t msg_size>
size_t RingQueue<msg_size>::GetSize() const noexcept
{
    return m_QueueSize;
}
