...
using RingQueue__ = RingQueue<64>;


#ifdef DEBUG_MODE
TEST(RingQueueDeathTest, Create) {
    EXPECT_DEATH({
        RingQueue__ queue(0);
...
protected:
    ...
    RingQueue__ smallest_queue;
    RingQueue__ queue;

    char buffer[RingQueue__::max_message_size] = {0};
    static const size_t message_len = sizeof(MSG_1);
    const char message[message_len] = MSG_1;
};


TEST_F(RingQueueTest, CreateQueueOfSize1) {
    EXPECT_EQ(smallest_queue.GetSize(), 1);    
}


TEST_F(RingQueueTest, SingleReadWrite) {
...
TEST_F(RingQueueTest, WriteAndReadToStream) {
...
TEST_F(RingQueueTest, TestFullandEmpty) {
...
TEST_F(RingQueueTest, TestIsAlive) {
...
TEST_F(RingQueueTest, TestShutDownReaders) {
    EXPECT_TRUE(queue.isAlive());
    queue.ShutDown();
    EXPECT_FALSE(queue.isAlive());
}
...
TEST_F(RingQueueTest, TestAvailable) {
...
TEST_F(RingQueueTest, TestAvailable2) {
...
TEST_F(RingQueueTest, TestReadWriteEmplace)
{
    struct Data
    {
        int i;
...
TEST_F(RingQueueTest, TestUserCallbacks)
{
    char src[] = "abcdefg";
...
public:
    using thread_fun = void (MultiThreadTest::* )(RingQueue__ &);

    void writer(RingQueue__ & queue)
    {
...
protected:
    ...
    RingQueue__ queue, big_queue;

    void LaunchWriters(thread_fun f, RingQueue__ &queue)
    {
    ...
    void LaunchReaders(thread_fun f, RingQueue__ &queue)
    {
